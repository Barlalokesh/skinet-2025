 ...
 async getConfirmationToken() {
  try {
    if (Object.values(this.completionStatus()).every(status => status === true)) {
      const token = await this.stripeService.createConfirmationToken();
  ...
  async onStepChange(event: StepperSelectionEvent) {
    if (event.selectedIndex === 1 ) {
      if (this.saveAddress) {
        const address = await this. getAddressFromStripedAddress() as Address;
...
if (result.paymentIntent?.status === 'succeeded') {
  ...
  private async createOrderModel(): Promise<OrderToCreate> {
    ...
    const shippingAddress = await this.getAddressFromStripedAddress() as ShippingAddress;
   ...
   return {
    ...
    private async getAddressFromStripedAddress(): Promise<Address|ShippingAddress | null> {
