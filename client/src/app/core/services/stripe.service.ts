...
async createConfirmationToken(): Promise<ConfirmationToken> {
  const stripe = await this.getStripeInstance();
  const elements = await this.initializeElements();
  ...
  if (!tokenResult.confirmationToken) {
    throw new Error('No confirmation token returned.');
  }

  return tokenResult.confirmationToken;
}
